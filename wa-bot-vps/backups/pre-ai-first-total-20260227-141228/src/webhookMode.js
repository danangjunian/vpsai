...
function toWebhookResult(saveResult) {
  ...
  const status = String(saveResult.status || "").trim().toUpperCase();
  if (!status || status === "SINGLE" || status === "MULTIPLE" || status === "NOT_FOUND") {
    return "OK";
  }
  if (status === "ERROR") {
    return "ERROR_" + String(saveResult.error || "ERROR");
  }
  return "OK";
}
