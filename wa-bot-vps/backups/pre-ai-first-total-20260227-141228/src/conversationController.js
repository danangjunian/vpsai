class ConversationController {
  ...
  async processIncomingText(text, messageMeta) {
    ...
    if (needsClarification) {
      const question = String(decision.clarification_question || "").trim() ||
        await this.aiEngine.composeClarificationQuestion({ userMessage: userMessage, decision: decision });
  ...
  async buildDailyExpenseReminderReply() {
    return this.aiEngine.composeReminder({
      purpose: "daily_expense_reminder"
    });
  }
  ...
  getPending_(sessionKey) {
    ...
    if (!item) return null;
    if (Date.now() - Number(item.askedAt || 0) > this.pendingTtlMs) {
      this.pendingClarifications.delete(key);
      return null;
    }
    return {
      asked_at: new Date(Number(item.askedAt || Date.now())).toISOString(),
